...
最良評価回数 2log(z)+min(m,n)である。

mまたはnは大体においてzより小さくなりうるので、
たとえば f (x,y) = 2^x + 3^y の場合であれば
最悪の場合でもO(log(z))しかかからないアルゴリズムとなる。
